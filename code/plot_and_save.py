...
def plot_results(predictions_rescaled, actuals_rescaled, save_dir, threshold=10, mode='test',
                 xlim=(-4, 4), ylim=(-4, 4)):
    ...
    epsilon, beta, nrmse, rmsle, mape, bias, mae = calculate_metrics(filtered_predictions, filtered_actuals, threshold)
    ...
    plt.legend(
        title=(f'MAE = {mae:.2f}, NRMSE = {nrmse:.2f}, RMSLE = {rmsle:.2f}\n'
               f'Bias = {bias:.2f}, Slope = {slope:.2f}\n'
               f'MAPE = {mape:.2f}%, ε = {epsilon:.2f}%, β = {beta:.2f}%'),
        fontsize=12, title_fontsize=10, prop={'family': 'Ubuntu'}
    )
...
def plot_results_with_density(predictions_rescaled, actuals_rescaled, save_dir,
                              threshold=10, mode='test_density',
                              xlim=(-4, 4), ylim=(-4, 4),
                              cmap='viridis',
                              tick_min=-4, tick_max=4, tick_step=1):
    os.makedirs(save_dir, exist_ok=True)
    ...
    # Calculate metrics
    epsilon, beta, nrmse, rmsle, mape, bias, mae = calculate_metrics(p_f, a_f, threshold)
    ...
    sc = plt.scatter(log_a_f, log_p_f, c=z, s=30, cmap=cmap,
...
def save_results_to_excelV2(ids, actuals, predictions, file_path,
                          dates=None, lat=None, lon=None, area=None):
    """
...
def save_results_from_excel_for_testV2(predictions, actuals,
                                     sample_ids, dates,
                                     original_excel_path, save_dir,
                                     lat=None, lon=None, area=None):
...
def save_results_from_excel_for_test(predictions, actuals, sample_ids, dates, original_excel_path, save_dir):
    os.makedirs(save_dir, exist_ok=True) 

    filename = os.path.basename(original_excel_path)
    dataset_name = os.path.splitext(filename)[0]

    save_results_to_excel(sample_ids, actuals, predictions,
                          os.path.join(save_dir, f'{dataset_name}.xlsx'), dates=dates)
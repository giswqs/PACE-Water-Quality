...
class MoE(nn.Module):
    def __init__(self, input_size, output_size, num_experts, noisy_gating=False, k=4):
        ...
        self.experts = nn.ModuleList(
            [VAE(self.input_size, self.output_size) for i in range(self.num_experts)]
        )
...
class TokenMoEModel(nn.Module):
    ...
    def forward(self, x):
        ...
        token_outputs = moe_output.reshape(
            batch_size, -1
        )  # [B, output_size * num_tokens]
        ...
        return {"pred_y": pred_y, "mu": mu, "log_var": log_var, "moe_loss": moe_loss}
